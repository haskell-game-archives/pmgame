...
renderHighScoreDisplay gm =
  let scores = gm ^. T.highscores
   ...
   in case gm ^. T.mode of
        ...
        _otherwise -> renderHighScores scores
...
renderHighScores xs
  ...
  where
    hdr = hCenter . withAttr "highScore" . txt $ "High Scores"
...
highScoreToWidget = hCenter . withAttr "info" . str . go
  where
    go (name, s)
      | length name > available = truncated
