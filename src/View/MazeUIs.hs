...
renderMaze tiler gm = vBox . renderTiles . M.toLists . tiler $ gm
  where
    renderTiles = map (hBox . map (renderTile gm))
...
tileGhosts gm m0 = foldl' (\m (p, t) -> M.setElem t p m) m0 gs
  where
    gs = [(g ^. T.gpos, tileGhost gm g) | g <- gm ^. T.ghosts]
...
renderMessage gm =
  let levelMsg = "Level " ++ show (gm ^. T.level)
   in case gm ^. T.msg of
        ...
        _otherwise -> withAttr "info" . str $ levelMsg
...
renderHeader gm = vLimit 3 . vBox $ [row1, row2, row3]
  where
    row1 = padLeft Max . withAttr "score" . txt $ "High"
