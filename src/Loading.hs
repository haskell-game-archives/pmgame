...
getFruitDelay = randomR (tmin, tmax)
  where
    tmin = toMicroSeconds @Double 5
    tmax = toMicroSeconds @Double 25
...
resolveWall xs (r, c) x
  ...
  | otherwise = Wall "═"
  where
    nw = verticalLink x . lookup (r -1, c) $ xs
