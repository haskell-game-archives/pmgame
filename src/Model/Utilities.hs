...
powerDuration lvl
  | lvl > 19 = 0
  | otherwise = toMicroSeconds @Double $ 10 - fromIntegral (abs lvl) / 2

tickPeriod :: Time
-- ^ Time between clock ticks.
tickPeriod = toMicroSeconds @Double 0.225
...
transitionDelay = toMicroSeconds @Double 1.5

-- Unexported

messageDuration :: Time
-- ^ Length of time messages are displayed.
messageDuration = toMicroSeconds @Double 3
...
updateHighScores gm = gm & T.highscores %~ addHighScore score
  where
    name = unlines . getEditContents $ gm ^. T.hsedit
...
formatPlayerName x
  ...
  | otherwise = goodName
  where
    goodName = filter (not . isControl) x
...
highScore gm
  ...
  | otherwise = hs
  where
    hs = snd . maximumBy (comparing snd) $ gm ^. T.highscores
...
fruitDuration Cherry = toMicroSeconds @Double 60
fruitDuration Strawberry = toMicroSeconds @Double 50
fruitDuration Orange = toMicroSeconds @Double 40
fruitDuration Apple = toMicroSeconds @Double 30
fruitDuration Melon = toMicroSeconds @Double 20
fruitDuration Galaxian = toMicroSeconds @Double 15
fruitDuration Bell = toMicroSeconds @Double 10
fruitDuration Key = toMicroSeconds @Double 10
...
moveFrom m p d =
  let go (x0, y0) (x1, y1) = (x0 + x1, y0 + y1)
...
findPathBetween m p0 p1
  ...
  where
    oneIsWall = isWall (m ! p0) || isWall (m ! p1)
    go [] = []
...
getNxtPoints m xs = filter (not . flip elem xs) . go
  where
    go (r, c) = filter chk [(r, c -1), (r, c + 1), (r -1, c), (r + 1, c)]
