...
endPlay gm
  ...
  where
    moreLives = gm ^. T.oneups > 0

resetPacMan :: PacMan -> PacMan
resetPacMan p =
  let (pos0, dir0) = p ^. T.pstrt
   in p & T.ppos .~ pos0
        & T.pdir .~ dir0

resetGhost :: Ghost -> Ghost
resetGhost g =
  let (pos0, dir0) = g ^. T.gstrt
   in g & T.gpos .~ pos0
        & T.gdir .~ dir0
        & T.gstate .~ Normal
        & T.gpathback .~ []
...
updateGame gm0 gm1
  ...
  where
    levelFinished = gm1 ^. T.npellets == 0
...
updateFruitTimes gm = gm & T.fruit %~ fmap go
  where
    dt = gm ^. T.dtime
    go frt
      | frt ^. T.fdelay > 0 = frt & T.fdelay %~ subtract dt
...
updateModeTimers gm = gm & T.mode %~ go
  where
    go (PwrRunning t0) = PwrRunning (t0 - gm ^. T.dtime)
...
ghostCaptures gm0 gm1 =
  let p0 = gm0 ^. T.pacman . T.ppos
      p1 = gm1 ^. T.pacman . T.ppos
      gs = zip (gm0 ^. T.ghosts) (gm1 ^. T.ghosts)
   in [ g1 | (g0, g1) <- gs, g0 ^. T.gstate /= EyesOnly, isCapture (p0, p1) (g0 ^. T.gpos, g1 ^. T.gpos)
      ]
...
movePlayer gm
  ...
  | otherwise =
    let p0 = gm ^. T.pacman . T.ppos
        d0 = gm ^. T.pacman . T.pdir
        m = gm ^. T.maze
        p1 = moveFrom m p0 d0
        gm' =
          gm & T.pacman . T.ppos .~ p1
...
isPlayerWaiting gm = dt < playerWaitTime
  where
    dt = gm ^. T.time - gm ^. T.pacman . T.ptlast

eatPellet :: Game -> Point -> Game
eatPellet gm p =
  gm & T.pacman . T.ppos .~ p
    & T.pacman . T.ptlast .~ (gm ^. T.time)
...
moveGhosts gm =
  let (gs, r) = foldr (moveGhost gm) start $ gm ^. T.ghosts
      start = ([], gm ^. T.rgen)
   in gm & T.ghosts .~ gs
        & T.rgen .~ r
...
moveGhost gm g0 (gs, r0) = case g0 ^. T.gstate of
  ...
  _otherwise -> moveWholeGhost gm g0 (gs, r0)
...
moveWholeGhost gm g0 (gs, r0)
  ...
  | otherwise = (g1 : gs, r1)
  where
    (ds, r1) = proposeDirections gm g0 r0
...
chooseDirection _gm g [] = (g ^. T.gdir, g ^. T.gpos)
